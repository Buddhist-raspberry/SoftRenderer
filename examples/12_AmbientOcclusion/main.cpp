...
int main(int argc, char** argv) {
	...
	shader->specularColor = VecColor::White*0.2f;
	shader->gloss = 5.0f;
	mesh->mShader = shader;

	pipeline->ambient = new AmbientLight(VecColor::White, 0.5f);
	pipeline->AddLight(new DirectionalLight(VecColor::White, 0.0f, glm::vec3(0, 1, 0)));

	pipeline->SetBGColor(VecColor::LightSlateBlue);
	pipeline->Render(mesh, colorbuffer);

	/*保存为图片*/
	std::string result_name = "12_AmbientOcclusion.png";
	stbi_write_png(result_name.c_str(), width, height, 4, colorbuffer, 0);


	/*显示到窗口程序*/
	app = new SRApp();
	app->Init("12_AmbientOcclusion", width, height);
